...
def index():
    ...
    return render_template("index.html", database = transactions_db, cash = round(cash, 2))
...
def buy():
    ...
    if request.method == "GET":
        ...
        if user_cash < transaction_value:
            return apology("Not enough cash on account.")
...
def register():
    ...
    if request.method == "GET":
        ...
        if not confirmation:
            return apology("Confirm your password. Can not be blank.")
...
def sell():
    ...
    if request.method == "GET":
        ...
        return render_template("sell.html", symbols = [row["symbol"] for row in symbols_user])
         ...
         user_shares = db.execute("SELECT shares from transactions where user_id = :id AND symbol =:symbol GROUP BY symbol", id=user_id, symbol=stock["symbol"])
...
def add_cash():
    ...
    if request.method == "GET":
        ...
        if not new_cash:
            return apology("Should be more than 0.")
